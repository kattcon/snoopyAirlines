CREATE TABLE dbo.PurchaseOrderRoute (
    PurchaseOrderId INT             NOT NULL,
    SequenceNumber  INT             NOT NULL,
    RouteId         INT             NOT NULL,
    IntendedDate    DATE            NULL,

    CONSTRAINT pk_purchase_order_route                  PRIMARY KEY (PurchaseOrderId, SequenceNumber),
    CONSTRAINT fk_purchase_order_route_purchase_order   FOREIGN KEY (PurchaseOrderId)
                                                        REFERENCES dbo.PurchaseOrder(Id),
    CONSTRAINT fk_purchase_order_route_route            FOREIGN KEY (RouteId)
                                                        REFERENCES dbo.[route](id),
    CONSTRAINT ck_purchase_order_route_sequence         CHECK (SequenceNumber >= 1)
);
GO

CREATE INDEX ix_purchase_order_route_route
    ON dbo.PurchaseOrderRoute (RouteId);
GO

INSERT INTO dbo.PurchaseOrderRoute (PurchaseOrderId, SequenceNumber, RouteId, IntendedDate)
SELECT Id, 1, RouteId, IntendedDate
...
CREATE TABLE dbo.itinerary (
    ...
    CONSTRAINT pk_itinerary                 PRIMARY KEY (booking_guid, sequence_number),
    CONSTRAINT fk_itinerary_booking         FOREIGN KEY (booking_guid)
                                            REFERENCES dbo.booking(guid),
    CONSTRAINT fk_itinerary_flight          FOREIGN KEY (flight_guid)
                                            REFERENCES dbo.flight(guid),
    CONSTRAINT ck_itinerary_sequence        CHECK (sequence_number >= 1),
    CONSTRAINT uq_itinerary_booking_flight  UNIQUE (booking_guid, flight_guid)
);
GO

CREATE INDEX ix_itinerary_flight
    ON dbo.itinerary (flight_guid)
    INCLUDE (booking_guid);
GO

INSERT INTO
    dbo.itinerary (booking_guid, sequence_number, flight_guid)
SELECT
    guid, 1, flight_guid
FROM
    dbo.booking;
...
DROP PROCEDURE IF EXISTS dbo.book;
GO

DROP FUNCTION IF EXISTS dbo.GetPurchaseOrderDetails;
GO

DROP FUNCTION IF EXISTS dbo.remaining_seats;
GO

ALTER TABLE dbo.booking DROP CONSTRAINT fk_booking_flight;
GO

ALTER TABLE dbo.booking DROP COLUMN flight_guid;
GO

ALTER TABLE dbo.PurchaseOrder DROP COLUMN RouteId;
GO

ALTER TABLE dbo.PurchaseOrder DROP COLUMN IntendedDate;
GO
...
BEGIN
    ...
    SELECT @total_amount = SUM(unit_price) * @passenger_count
    FROM @legs;
