CREATE TABLE dbo.partner_airline (
    ...
    CONSTRAINT ck_partner_airline_hostck_partner_airline_api_key CHECK (NULLIF(LTRIM(RTRIM(host)), '') IS NOT NULL),
    CONSTRAINT  CHECK (NULLIF(LTRIM(RTRIM(api_key)), '') IS NOT NULL)
);
GO