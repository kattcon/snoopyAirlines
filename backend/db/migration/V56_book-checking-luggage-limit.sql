...
BEGIN
    ...
    -- Validar límites de equipaje por pasajero
    IF EXISTS (
        SELECT 1
        FROM dbo.passenger_exceeds_baggage_limit(@purchase_order_id)
        WHERE ExceedsCarryOn = 1
    )
    BEGIN
        THROW 50000, 'One or more passengers exceed the carry-on baggage weight limit.', 1;
    END;

    IF EXISTS (
        SELECT 1
        FROM dbo.passenger_exceeds_baggage_limit(@purchase_order_id)
        WHERE ExceedsChecked = 1
    )
    BEGIN
        THROW 50000, 'One or more passengers exceed the checked baggage weight limit.', 1;
    END;
