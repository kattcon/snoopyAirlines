...
RETURN (
    SELECT
        ...
        -- Ciudad de salida: viene de city (interno) o directo de flight_external
        COALESCE(dc.name, fe.departure_city)           AS DepartureCity,

        -- Ciudad de llegada: viene de city (interno) o directo de flight_external
        COALESCE(ac.name, fe.arrival_city)             AS ArrivalCity,

        -- Codigo aeropuerto salida
        COALESCE(da.code, fe.departure_airport_code)   AS DepartureAirportCode,

        -- Codigo aeropuerto llegada
        ...
        -- Duracion: solo disponible para vuelos internos
        r.duration_minutes                             AS DurationMinutes,

        -- Aeronave: solo disponible para vuelos internos
    ...
    -- Vuelo interno: tiene route_id → joins completos a route/aeropuerto/ciudad/avion
    ...
    -- Vuelo externo: datos de aeropuerto/ciudad desnormalizados en la propia tabla
    LEFT JOIN dbo.flight_external fe    ON fe.flight_guid           = f.guid

    WHERE
        b.confirmation_code = UPPER(LTRIM(RTRIM(@confirmation_code)))

        -- El cliente debe ingresar exactamente la ultima palabra de card_holder_name,
        -- que siempre corresponde al apellido (segundo apellido si tiene dos, o unico
        -- apellido si tiene uno). Se extrae con REVERSE + CHARINDEX
        AND REVERSE(
                LEFT(
